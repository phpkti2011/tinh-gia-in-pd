...
> **Cập nhật: 2026-09-19 (TASK-DECAL-CONTENT-SURCHARGE)** — thêm **phụ thu nhiều nội dung**, bê cơ chế từ In KTS khổ nhỏ (`calculatePrintContentSurcharge`). Một đơn in nhiều mẫu khác nhau tốn công dàn trang, canh máy hơn. Config `contentSurcharge` (**optional** — thiếu ⇒ 0% ⇒ giá không đổi) gồm `singleContentPercent` (áp riêng khi số nội dung = đúng số lượng, **bỏ qua bảng bậc**) + `tiers: [{min, max, percent}]` (mặc định 4-9→10%, 10-14→20%, 15-25→30%, 26+→35%; khoảng trống 2-3 nội dung là **cố ý** không phụ thu). **Khác small-print: % lưu dạng SỐ NGUYÊN** (10 = 10%) cho khớp mọi field `percent` khác của decal, không dùng phân số 0.1. Hàm thuần `getContentSurchargePercent(quantity, contentCount, config)` (pricing.js). Nhân **sau** % khổ và % bế demi, **trước** chiết khấu: `((in + vật liệu + cán×film%) × %khổ) × %bế demi × %nội dung` → chiết khấu/giá sàn → tròn nghìn. Áp cho **cả 2 chế độ** (tem lẻ so với số con, tờ sticker so với số tờ). `contentCount` là tham số vị trí cuối, **mặc định 1** ⇒ mọi golden test cũ giữ nguyên số. Ô nhập "Số nội dung in" dùng chung 2 chế độ; ô giá hiện badge `+N% nội dung`. Schema bump `1.8.0`.
