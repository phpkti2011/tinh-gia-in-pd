    ...
    <div className="group relative flex justify-center items-center h-full w-full">
        ...
        <div className="absolute opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900 text-white text-xs rounded py-1 px-2 pointer-events-none z-10 bottom-full mb-1 border border-gray-600 shadow-xl whitespace-nowrap">
            ...
            <svg
                ...
                xmlSpace="preserve"
            >
                <polygon className="fill-current border-gray-600" points="0,0 127.5,127.5 255,0" />
            </svg>
        </div>
    </div>
);

export function LargeSheetVisualizer({
    largeW,
    largeH,
    cutW,
    cutH,
    ...
    const isSwapped =
        Math.floor(largeW / cutW) * Math.floor(largeH / cutH) <
    ...
    if (!largeW || !largeH)
        return <div className="text-gray-500 italic p-4 text-center">Không có dữ liệu khổ lớn</div>;
        ...
        <div className="flex flex-col items-center">
            ...
            >
                ...
                <div
                    ...
                    style={{
                        backgroundImage:
                            'repeating-linear-gradient(45deg, #1f2937 25%, transparent 25%, transparent 75%, #1f2937 75%, #1f2937), repeating-linear-gradient(45deg, #1f2937 25%, #374151 25%, #374151 75%, #1f2937 75%, #1f2937)',
...
export function PrintSheetVisualizer({
    printW,
    printH,
    printableArea,
    prodW,
    prodH,
    productsPerSheet: _productsPerSheet,
    spacing,
}) {
    let rects = [];
    ...
    if (!printW || !printH)
        return <div className="text-gray-500 italic p-4 text-center">Không có dữ liệu tờ in</div>;
        ...
        <div className="flex flex-col items-center">
            ...
            >
                ...
                <div
                    ...
                    style={{
                        left: printableStartX,
                        top: printableStartY,
                        width: printableScaleW,
                        height: printableScaleH,
                    }}
                >
                    <div className="absolute -top-4 -left-0 text-[9px] text-red-500 tracking-wider">
                        VÙNG IN MAXIMUM
                    </div>
                </div>
                ...
                {/* Product Layout Grid */}
                {cols === 0 && rows === 0 ? (
                    <div className="absolute bg-black/60 text-white px-4 py-2 rounded-lg font-bold">
                        Không Vừa Thêm Sản Phẩm
                    </div>
                ) : (
                    rects.map((r) => (
                        <div
                        ...
                        >
                            ...
                            <Tooltip text={`Sản phẩm: ${itemW_final} x ${itemH_final} cm`}>
                                <span className="text-blue-900 font-black drop-shadow-[0_1px_1px_rgba(255,255,255,1)] text-xs sm:text-base z-10">
                                    {r.idx}
                                </span>
                            </Tooltip>
                        </div>
