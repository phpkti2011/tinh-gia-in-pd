...
export default function LPSettingsPanel({ config, onSave, onCancel }) {
    ...
    const updateMatName = (key, name) =>
        updateConfig((c) => {
            c.MATERIAL_TYPES[key].name = name;
        });
    const updateMatOption = (key, idx, field, val) =>
        updateConfig((c) => {
            c.MATERIAL_TYPES[key].options[idx][field] = val;
        });
    const addMatOption = (key) =>
        updateConfig((c) => {
            c.MATERIAL_TYPES[key].options.push({ width: 1.07, printPrice: 0, materialPrice: 0 });
        });
    const delMatOption = (key, idx) =>
        updateConfig((c) => {
    ...
    const addMaterial = () => {
        ...
        const k = matKey.trim();
        if (localConfig.MATERIAL_TYPES[k]) {
    ...
    const delMaterial = (key) => {
        ...
        updateConfig((c) => {
            delete c.MATERIAL_TYPES[key];
        });
    };

    // --- LAMINATION_TYPES helpers ---
    const updateLamName = (key, name) =>
        updateConfig((c) => {
            c.LAMINATION_TYPES[key].name = name;
        });
    const updateLamOption = (key, idx, field, val) =>
        updateConfig((c) => {
            c.LAMINATION_TYPES[key].options[idx][field] = val;
        });
    const addLamOption = (key) =>
        updateConfig((c) => {
            c.LAMINATION_TYPES[key].options.push({ width: 1.07, price: 0 });
        });
    const delLamOption = (key, idx) =>
        updateConfig((c) => {
            c.LAMINATION_TYPES[key].options.splice(idx, 1);
        });
    const addLamination = () => {
        const k = prompt('Nhập key cán màng (vd: mang_bong):');
        if (!k || k.trim() === '') return;
        const key = k.trim();
        if (localConfig.LAMINATION_TYPES[key]) {
            alert('Key đã tồn tại!');
            return;
        }
        ...
        if (!name) return;
        updateConfig((c) => {
            c.LAMINATION_TYPES[key] = { name: name.trim(), options: [{ width: 1.07, price: 0 }] };
        });
    };
    const delLamination = (key) => {
        if (!confirm(`Xóa cán màng "${localConfig.LAMINATION_TYPES[key].name}"?`)) return;
        updateConfig((c) => {
            delete c.LAMINATION_TYPES[key];
        });
    };

    // --- FORMEX helpers ---
    const updateFormex = (key, val) =>
        updateConfig((c) => {
            c.FORMEX_OPTIONS[key].price = val;
        });

    // --- FORMEX_DISCOUNT_TIERS helpers ---
    const updateDiscountTier = (idx, field, val) =>
        updateConfig((c) => {
            c.FORMEX_DISCOUNT_TIERS[idx][field] = val;
        });
    const addDiscountTier = () =>
        updateConfig((c) => {
            c.FORMEX_DISCOUNT_TIERS.push({ minArea: 0, maxArea: Infinity, discount: 0 });
        });
    const delDiscountTier = (idx) =>
        updateConfig((c) => {
    ...
    const updateFinishing = (field, val) =>
        updateConfig((c) => {
            c.FINISHING_PRICES[field] = val;
        });
    const updateDieCut = (field, val) =>
        updateConfig((c) => {
            c.FINISHING_PRICES.dieCutting[field] = val;
        });
