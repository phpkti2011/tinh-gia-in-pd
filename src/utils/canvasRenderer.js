export function drawLargeSheet(
    canvas,
    largeW,
    largeH,
    cutW,
    ...
    let totalSheetsOnThisLarge =
        typeof sheetsPerLarge === 'number' && sheetsPerLarge > 0 ? sheetsPerLarge : 1;
    ...
    if (layout && layout.length > 0) {
        ...
        if (totalSheetsOnThisLarge > 0 && cutW > 0 && cutH > 0) {
            ...
            const numCols = isSwapped
                ? cutH > 0
                    ? Math.floor(largeW / cutH)
                    : 0
                : cutW > 0
                  ? Math.floor(largeW / cutW)
                  : 0;
            const numRows = isSwapped
                ? cutW > 0
                    ? Math.floor(largeH / cutW)
                    : 0
                : cutH > 0
                  ? Math.floor(largeH / cutH)
                  : 0;
    ...
    const neededLargeSheets =
        totalSheetsOnThisLarge > 0 ? Math.ceil(neededPrintSheets / totalSheetsOnThisLarge) : 0;
    ctx.fillStyle = '#d1d5db';
    ctx.font = 'bold 16px Inter';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'bottom';
    const largeSheetText =
        totalSheetsOnThisLarge === 1 ? 'tờ in' : `tờ lớn (${totalSheetsOnThisLarge} tờ/lớn)`;
    ctx.fillText(
        `Cần ${neededPrintSheets} tờ in   ⇒  dùng ${neededLargeSheets} ${largeSheetText}`,
        canvasW / 2,
        startY - 30
    );
    ctx.font = '14px Inter';
...
export function drawPrintSheet(
    ...
    ctx.textBaseline = 'bottom';
    ctx.fillText(
        `Tổng sp/tờ: ${productsPerSheet} — Tờ cuối cần: ${productsOnThisSheet} sp`,
        canvasW / 2,
        startY - 30
    );
    ctx.font = '14px Inter';
